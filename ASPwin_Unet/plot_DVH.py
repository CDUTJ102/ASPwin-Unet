...
def main(configs):
    ...
    for patient_name in list_pred_patients:
        ...
        plt.ylim(0, 100)
        plt.xlim(0, 80)
        plt.title('DVH', fontsize=16)
        plt.ylabel('Volume (%)', fontsize=16)
        plt.xlabel('Dose (Gy)', fontsize=16)
        plt.legend(loc='lower left', bbox_to_anchor=(0, 0), borderaxespad=0.5)
        plt.savefig(os.path.join(save_dir, patient_name + '.png'))
        plt.close(fig1)
...
if __name__ == '__main__':
    ...
    cfgs = load_config(CONFIG_PATH, config_name='C3D.yaml')
    main(cfgs)
